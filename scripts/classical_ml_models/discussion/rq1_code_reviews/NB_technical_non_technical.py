...
def run_pipelines (pipeline, parameters, pipeline_id):
	...
	with open ('output/NB_technical_non_technical_pipeline'+pipeline_id+'.csv', "a+") as f:
		f.write("%s;%s;%s;%s;%s;%s;%s;%s;%s\n" % ("fold", "dataset", "data", "model", "thread_id_email_id", "issue_id", "comment_id", "label_correct", "label_predicted"))
		for parameter in EDA_parameters:
			...
			for train, test in outer_cv.split(X, y):
				...
				for (index, row), item in zip(y_test.iteritems(), yhat):
					correct_label = "not_technical" if row==0 else "technical"
					predicted_label = "not_technical" if item==0 else "technical"
					thread_id_email_id = str(dataframe.loc[index, "thread_id"]) + "_" + str(dataframe.loc[index, "email_id"])

					f.write("%s;%s;%s;%s;%s;%s;%s;%s;%s\n" % (fold_nr, "rq1_lkml_technical_non_technical", "Code reviews dataset", "NB", thread_id_email_id, dataframe.loc[index, "thread_id"], dataframe.loc[index, "email_id"], correct_label, predicted_label))
				fold_nr = fold_nr + 1
