		...
		for parameter in EDA_parameters:
			...
			for train, test in outer_cv.split(X, y):
				...
				for (index, row), item in zip(y_test.iteritems(), yhat):
					...
					f.write("%s;%s;%s;%s;%s;%s;%s;%s;%s\n" % (fold_nr, "rq1_lkml_technical_non_technical", "Code reviews dataset", "CART", thread_id_email_id, dataframe.loc[index, "thread_id"], dataframe.loc[index, "email_id"], correct_label, predicted_label))
				fold_nr = fold_nr + 1
