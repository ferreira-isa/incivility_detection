...
def run_pipelines (pipeline, parameters, pipeline_id):
	...
	EDA_parameters = [{'alpha_sr': 0.2, 'alpha_ri': 0.1, 'alpha_rs': 0.05, 'alpha_rd': 0.05, 'num_aug': 16}]
		...
		for parameter in EDA_parameters:
			...
			for train, test in outer_cv.split(X, y):
				...
				for (index, row), item in zip(y_test.iteritems(), yhat):
					...
					f.write("%s;%s;%s;%s;%s;%s;%s;%s;%s;%s\n" % (fold_nr, "rq1_lkml_civil_uncivil", "Code reviews dataset", "SVM", thread_id_email_id, dataframe.loc[index, "thread_id"], dataframe.loc[index, "email_id"], correct_label, predicted_label, dataframe.loc[index, "quotation_tbdf"]))
				fold_nr = fold_nr + 1
