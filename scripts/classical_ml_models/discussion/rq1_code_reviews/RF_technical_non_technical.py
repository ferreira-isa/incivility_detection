	...
	with open ('output/RF_technical_non_technical_pipeline'+pipeline_id+'.csv', "a+") as f:
		...
		for parameter in EDA_parameters:
			...
			for index, row in y.iteritems():
				...
				f.write("%s;%s;%s;%s;%s;%s;%s;%s\n" % ("rq1_lkml_technical_non_technical", "Code reviews dataset", "RF", thread_id_email_id, dataframe.loc[index, "thread_id"], dataframe.loc[index, "email_id"], correct_label, predicted_label))
