		...
		for parameter in EDA_parameters:
			...
			for train, test in outer_cv.split(X, y):
				...
				for (index, row), item in zip(y_test.iteritems(), yhat):
					...
					f.write("%s;%s;%s;%s;%s;%s;%s;%s;%s;%s\n" % (fold_nr, "rq1_gh_civil_uncivil", "Issues dataset", "KNN", thread_id_email_id, dataframe.loc[index, "issue_id"], dataframe.loc[index, "comment_id"], correct_label, predicted_label, dataframe.loc[index, "quotation_tbdf"]))
				fold_nr = fold_nr + 1
