...
non_technical_dataset = "../../data/gh/3_features_dataset/non_technical.csv"

technical_data = read_csv(technical_dataset, "technical", "gh")
non_technical_data = read_csv(non_technical_dataset, "non_technical", "gh")

dataframe = pd.concat([technical_data, non_technical_data], ignore_index=True)
		...
		for parameter in EDA_parameters:
			...
			for train, test in outer_cv.split(X, y):
				...
				for (index, row), item in zip(y_test.iteritems(), yhat):
					...
					thread_id_email_id = str(dataframe.loc[index, "issue_id"]) + "_" + str(dataframe.loc[index, "comment_id"])

					f.write("%s;%s;%s;%s;%s;%s;%s;%s;%s\n" % (fold_nr, "rq1_gh_technical_non_technical", "Issues dataset", "SVM", thread_id_email_id, dataframe.loc[index, "issue_id"], dataframe.loc[index, "comment_id"], correct_label, predicted_label))
				fold_nr = fold_nr + 1
