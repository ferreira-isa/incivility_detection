...
dataframe = pd.concat([civil_data_gh, uncivil_data_gh], ignore_index=True)
...
def run_pipelines (pipeline, parameters, pipeline_id):
	...
	with open ('output/CART_civil_uncivil_pipeline'+pipeline_id+'.csv', "a+") as f:
		f.write("%s;%s;%s;%s;%s;%s;%s;%s;%s;%s\n" % ("fold", "dataset", "data", "model", "thread_id_email_id", "issue_id", "comment_id", "label_correct", "label_predicted", "quotation_tbdf"))
		for parameter in EDA_parameters:
			...
			for train, test in outer_cv.split(X, y):
				...
				for (index, row), item in zip(y_test.iteritems(), yhat):
					...
					predicted_label = "uncivil" if item==0 else "civil"
					thread_id_email_id = str(dataframe.loc[index, "issue_id"]) + "_" + str(dataframe.loc[index, "comment_id"])

					f.write("%s;%s;%s;%s;%s;%s;%s;%s;%s;%s\n" % (fold_nr, "rq1_gh_civil_uncivil", "Issues dataset", "CART", thread_id_email_id, dataframe.loc[index, "issue_id"], dataframe.loc[index, "comment_id"], correct_label, predicted_label, dataframe.loc[index, "quotation_tbdf"]))
				fold_nr = fold_nr + 1
