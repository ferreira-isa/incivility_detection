	...
	with open ('output/KNN_civil_uncivil_pipeline'+pipeline_id+'.csv', "a+") as f:
		...
		for parameter in EDA_parameters:
			...
			for index, row in y.iteritems():
				...
				f.write("%s;%s;%s;%s;%s;%s;%s;%s;%s\n" % ("rq3_train_gh_test_lkml_civil_uncivil", "Train on issues and test on code reviews", "KNN", thread_id_email_id, dataframe.loc[index, "thread_id"], dataframe.loc[index, "email_id"], correct_label, predicted_label, dataframe.loc[index, "quotation_tbdf"]))
