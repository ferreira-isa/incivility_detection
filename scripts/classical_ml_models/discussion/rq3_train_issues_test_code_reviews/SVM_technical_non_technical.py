...
def run_pipelines (pipeline, parameters, pipeline_id):
	...
	EDA_parameters = [{'alpha_sr': 0.05, 'alpha_ri': 0.05, 'alpha_rs': 0.05, 'alpha_rd': 0.05, 'num_aug': 4}]

	with open ('output/SVM_technical_non_technical_pipeline'+pipeline_id+'.csv', "a+") as f:
		f.write("%s;%s;%s;%s;%s;%s;%s;%s\n" % ("dataset", "data", "model", "thread_id_email_id", "issue_id", "comment_id", "label_correct", "label_predicted"))
		for parameter in EDA_parameters:
			
			X_train_augmented = gen_eda(X_gh, parameter["alpha_sr"], parameter["alpha_ri"], parameter["alpha_rs"], parameter["alpha_rd"], parameter["num_aug"])  
			...
			for index, row in y.iteritems():
				correct_label = "not_technical" if row==0 else "technical"
				predicted_label = "not_technical" if yhat[index]==0 else "technical"
				thread_id_email_id = str(dataframe.loc[index, "thread_id"]) + "_" + str(dataframe.loc[index, "email_id"])

				f.write("%s;%s;%s;%s;%s;%s;%s;%s\n" % ("rq3_train_gh_test_lkml_tech_non_tech", "Train on issues and test on code reviews", "SVM", thread_id_email_id, dataframe.loc[index, "thread_id"], dataframe.loc[index, "email_id"], correct_label, predicted_label))
