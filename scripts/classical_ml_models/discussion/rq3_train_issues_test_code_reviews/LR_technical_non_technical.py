	...
	with open ('output/LR_technical_non_technical_pipeline'+pipeline_id+'.csv', "a+") as f:
		...
		for parameter in EDA_parameters:
			...
			for index, row in y.iteritems():
				...
				f.write("%s;%s;%s;%s;%s;%s;%s;%s\n" % ("rq3_train_gh_test_lkml_tech_non_tech", "Train on issues and test on code reviews", "LR", thread_id_email_id, dataframe.loc[index, "thread_id"], dataframe.loc[index, "email_id"], correct_label, predicted_label))
