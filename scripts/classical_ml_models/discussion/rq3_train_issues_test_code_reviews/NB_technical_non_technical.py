...
parameters = {
    'features__text_vectorizer__vect__ngram_range': [(1,2)],
    'clf__alpha': [0.0],
}
...
def run_pipelines (pipeline, parameters, pipeline_id):
	...
	EDA_parameters = [{'alpha_sr': 0.2, 'alpha_ri': 0.05, 'alpha_rs': 0.05, 'alpha_rd': 0.05, 'num_aug': 4}
	                  ]
	...
	with open ('output/NB_technical_non_technical_pipeline'+pipeline_id+'.csv', "a+") as f:
		...
		for parameter in EDA_parameters:
			...
			for index, row in y.iteritems():
				...
				f.write("%s;%s;%s;%s;%s;%s;%s;%s\n" % ("rq3_train_gh_test_lkml_tech_non_tech", "Train on issues and test on code reviews", "NB", thread_id_email_id, dataframe.loc[index, "thread_id"], dataframe.loc[index, "email_id"], correct_label, predicted_label))
