	...
	with open ('output/SVM_technical_non_technical_pipeline'+pipeline_id+'.csv', "a+") as f:
		...
		for parameter in EDA_parameters:
			...
			for index, row in y_gh.iteritems():
				...
				f.write("%s;%s;%s;%s;%s;%s;%s;%s\n" % ("rq3_train_lkml_test_gh_tech_non_tech", "Train on code reviews and test on issues", "CART", thread_id_email_id, dataframe_gh.loc[index, "issue_id"], dataframe_gh.loc[index, "comment_id"], correct_label, predicted_label))
