	...
	with open ('output/LR_civil_uncivil_pipeline'+pipeline_id+'.csv', "a+") as f:
		...
		for parameter in EDA_parameters:
			...
			for index, row in y_gh.iteritems():
				...
				f.write("%s;%s;%s;%s;%s;%s;%s;%s;%s\n" % ("rq3_train_lkml_test_gh_civil_uncivil", "Train on code reviews and test on issues", "LR", thread_id_email_id, dataframe_gh.loc[index, "issue_id"], dataframe_gh.loc[index, "comment_id"], correct_label, predicted_label, dataframe_gh.loc[index, "quotation_tbdf"]))
