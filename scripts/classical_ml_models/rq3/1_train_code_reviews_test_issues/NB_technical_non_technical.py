...
def read_csv(filename, file_type, dataset):
	...
	# Deleting columns not useful for the model
		if (file_type == "technical") or (file_type == "non_technical"):
			...
			del df ["author_email"]
			del df ["author_role"]
			df['email_final_code'] = [0 if x=='uncivil' else 1 for x in df['email_final_code']]
			df.rename(columns={"quotation": "text_content"}, inplace=True)
			df.rename(columns={"email_final_code": "label"}, inplace=True)
...
technical_dataset = "../../data/lkml/3_features_dataset/technical.csv"
not_technical_dataset = "../../data/lkml/3_features_dataset/non_technical.csv"

technical_data = read_csv(technical_dataset, "technical", "lkml")
not_technical_data = read_csv(not_technical_dataset, "non_technical", "lkml")

dataframe = pd.concat([technical_data, not_technical_data], ignore_index=True)
dataframe = dataframe[(dataframe["time_previous_to_email"] >= 0)]
...
gh_technical_dataset = "../../data/gh/3_features_dataset/technical.csv"
gh_not_technical_dataset = "../../data/gh/3_features_dataset/non_technical.csv"

technical_data_gh = read_csv(gh_technical_dataset, "technical", "gh")
not_technical_data_gh = read_csv(gh_not_technical_dataset, "non_technical", "gh")

dataframe_gh = pd.concat([technical_data_gh, not_technical_data_gh], ignore_index=True)
...
def gen_eda(orig_df_train, alpha_sr, alpha_ri, alpha_rs, alpha_rd, num_aug=9):
	output_train_df = pd.DataFrame(columns=["text_content", "label", "is_first_author_thread", "nr_characters", "ratio_words_email_thread", "position_comment_thread", "is_last_comment", "time_start_to_email", "time_email_to_end", "time_previous_to_email", "time_email_to_next"])
	...
	for idx in orig_df_train.index:
		...
		position_comment_thread = orig_df_train.loc[idx, 'position_comment_thread']
		...
		d["position_comment_thread"] = [position_comment_thread for _ in range(len(aug_sentences))]
...
parameters = {
    'features__text_vectorizer__vect__ngram_range': [(1,1)],
    'clf__alpha': [1.0],
}
...
def run_pipelines (pipeline, parameters, pipeline_id):
	...
	EDA_parameters = [#{"alpha_sr": 0.2, "alpha_ri": 0.1, "alpha_rs": 0.05, "alpha_rd": 0.05, "num_aug": 8},
	                  #{"alpha_sr": 0.2, "alpha_ri": 0.1, "alpha_rs": 0.05, "alpha_rd": 0.05, "num_aug": 4},
	                  #{"alpha_sr": 0.2, "alpha_ri": 0.05, "alpha_rs": 0.05, "alpha_rd": 0.05, "num_aug": 8},
	                  #{"alpha_sr": 0.2, "alpha_ri": 0.05, "alpha_rs": 0.05, "alpha_rd": 0.05, "num_aug": 4},
	                  #{"alpha_sr": 0.05, "alpha_ri": 0.1, "alpha_rs": 0.05, "alpha_rd": 0.05, "num_aug": 8},
	                  {"alpha_sr": 0.05, "alpha_ri": 0.1, "alpha_rs": 0.05, "alpha_rd": 0.05, "num_aug": 4},
	                  #{"alpha_sr": 0.05, "alpha_ri": 0.05, "alpha_rs": 0.05, "alpha_rd": 0.05, "num_aug": 8},
	                  #{"alpha_sr": 0.05, "alpha_ri": 0.05, "alpha_rs": 0.05, "alpha_rd": 0.05, "num_aug": 4}
	                  ]

	with open ('output/NB_technical_non_technical_pipeline'+pipeline_id+'.csv', "a+") as f:
		...
		for parameter in EDA_parameters:
			...
			for train, test in outer_cv.split(X, y):
				...
				X_test = X_test.drop(['text_content'], axis=1)
				X_test = X_test.drop(['label'], axis=1)

				X_train_gh, X_test_gh = X_gh.iloc[train], X_gh.iloc[test]
				y_train_gh, y_test_gh = y_gh.iloc[train], y_gh.iloc[test]
				...
				precision = precision_score(y_gh, yhat, average='macro')
				recall = recall_score(y_gh, yhat, average='macro')
				f1 = f1_score(y_gh, yhat, average='macro')
				mcc = compute_mcc(y_gh, yhat)

				outer_precision.append(precision)
				outer_recall.append(recall)
				outer_f1.append(f1)
				outer_mcc.append(mcc)
				classification_scores = classification_report(y_gh, yhat, output_dict=True)
				#df_classification_scores = pd.DataFrame(classification_scores).transpose()

				# Misclassified cases
				misclassified = np.where(y_gh != yhat)
				rows = misclassified[0]
				columns = ["issue_id", "comment_id", "text_content", "label"]
				filtered_misclassified = dataframe_gh.loc[rows, columns].to_dict()
...
"""### Run pipelin3"""
#trun_pipelines (pipeline3, parameters, "3")
