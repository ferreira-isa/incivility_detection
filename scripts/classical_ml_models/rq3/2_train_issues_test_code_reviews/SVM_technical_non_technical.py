...
def run_pipelines (pipeline, parameters, pipeline_id):
	...
	with open ('output/SVM_technical_non_technical_pipeline'+pipeline_id+'.csv', "a+") as f:
		...
		for parameter in EDA_parameters:
			fold_nr = 1
			for train, test in outer_cv.split(X_gh, y_gh):
				print("Fold: ", fold_nr)
				print("EDA parameters: ", parameter)
				X_train, X_test = X_gh.iloc[train], X_gh.iloc[test]
				y_train, y_test = y_gh.iloc[train], y_gh.iloc[test]				
