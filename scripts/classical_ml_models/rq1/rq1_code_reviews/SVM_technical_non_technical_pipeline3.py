...
technical_dataset = "../data/features_dataset/technical.csv"
not_technical_dataset = "../data/features_dataset/non_technical.csv"
...
#'clf__C': (pow(2, -5), pow(2, -3), pow(2, -1), pow(2, 0), pow(2, 1), pow(2, 3), pow(2, 5), pow(2, 7), pow(2, 9), pow(2, 11), pow(2, 13), pow(2, 15)),

#'clf__gamma': (pow(2, -15), pow(2, -13), pow(2, -11), pow(2, -9), pow(2, -7), pow(2, -5), pow(2, -3), pow(2, -1), pow(2, 0), pow(2, 1), pow(2, 3)),

parameters = {
    'features__text_vectorizer__vect__ngram_range': ((1,1),(1,2)),  # unigrams or bigrams or trigrams
    'clf__C': (pow(2, -5), pow(2, -3), pow(2, -1), pow(2, 0), pow(2, 1)),
    'clf__gamma': (pow(2, -15), pow(2, -5), pow(2, -3), pow(2, -1), pow(2, 0), pow(2, 1)),
}
	...
	with open ('output/SVM_technical_non_technical_pipeline'+pipeline_id+'.csv', "a+") as f:
		...
		for parameter in EDA_parameters:
			...
			for train, test in outer_cv.split(X, y):
				...
				columns = ["thread_id", "email_id", "text_content", "label"]
				filtered_misclassified = dataframe.loc[rows, columns].to_dict()

				f.write("%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s\n" % (fold_nr, classification_scores["macro avg"]["precision"], classification_scores["macro avg"]["recall"], classification_scores["macro avg"]["f1-score"], mcc, classification_scores["macro avg"]["support"], str(parameter), str(classification_scores), str(best_model_score), str(best_model_params), str(filtered_misclassified)))S
