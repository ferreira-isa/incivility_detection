# -*- coding: utf-8 -*-
"""linux_civil1_5.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1cq214sKjk-QZFqErzGc6VAqF8IOtXCpE
"""
...
# ------------ GITHUB
# read CSVs
url_c = 'https://github.com/ahlraf/datasets/raw/main/gh_utf_civil.csv'
url_uc = 'https://github.com/ahlraf/datasets/raw/main/gh_utf_uncivil.csv'
...
# print(df_uc["label"].value_counts())

df_c.dropna(axis=0,how="any",inplace=True)
df_uc.dropna(axis=0,how="any",inplace=True)

df = pd.concat([df_c, df_uc], ignore_index=True)

#------------- LINUX
url_c1 = 'https://github.com/ahlraf/datasets/raw/main/utf_civil.csv'
url_uc1 = 'https://github.com/ahlraf/datasets/raw/main/utf_uncivil.csv'

# getting datasets
df_c1 = pd.read_csv(url_c1, header='infer', skip_blank_lines=True, encoding="utf-8")
df_uc1 = pd.read_csv(url_uc1, header='infer', skip_blank_lines=True, encoding="utf-8")

# print(df_c["label"].value_counts())
# print(df_uc["label"].value_counts())

df_c1.dropna(axis=0,how="any",inplace=True)
df_uc1.dropna(axis=0,how="any",inplace=True)

d1 = pd.DataFrame(['civil' for _ in range(df_c1.shape[0])], columns=['label'])
df_civ = pd.concat([df_c1['quotation'], d1], axis=1)

d2 = pd.DataFrame(['uncivil' for _ in range(df_uc1.shape[0])], columns=['label'])
df_unciv = pd.concat([df_uc1['quotation'], d2], axis=1)

test = pd.concat([df_civ, df_unciv], ignore_index=True)


df.rename(columns={"quotation":"text"}, inplace=True)
test.rename(columns={"quotation":"text"}, inplace=True)

df['label'] = [0 if x=='civil' else 1 for x in df['label']]
test['label'] = [0 if x=='civil' else 1 for x in test['label']]
...
args = TrainingArguments(
    ...
    save_strategy = "epoch",
    learning_rate=2.5497706126829216e-05,
    per_device_train_batch_size=4,
    per_device_eval_batch_size=batch_size,
    num_train_epochs=10,
    weight_decay=0.10822922685686845,
    warmup_steps=8,
...
print("\n\nEDA 0.2, 0.05, 0.05, 0.05, 8\n\n")

for fold_i in range(len(folds)):
  print("\nFold", fold_i+1, "\n")

  train_df, eval_df, test_df = folds[fold_i][0], folds[fold_i][1], test

  # ---------------------- EDA on train -----------------------
  # empty DataFrame
  output_train = pd.DataFrame(columns=["text", "label"])
  aug_train = gen_eda(train_df, output_train, 0.2, 0.05, 0.05, 0.05, 8)
