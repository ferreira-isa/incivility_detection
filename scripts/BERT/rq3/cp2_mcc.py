...
#------------- LINUX
url_nt = 'https://github.com/ahlraf/datasets/raw/main/lin_nontech_utf.csv'
url_t = 'https://github.com/ahlraf/datasets/raw/main/lin_tech_utf.csv'

# getting datasets
df_nt = pd.read_csv(url_nt, header='infer', skip_blank_lines=True, encoding="utf-8")
df_t = pd.read_csv(url_t, header='infer', skip_blank_lines=True, encoding="utf-8")

df_nt.dropna(axis=0,how="any",inplace=True)
df_t.dropna(axis=0,how="any",inplace=True)

test = pd.concat([df_nt, df_t], ignore_index=True)

df.rename(columns={"code":"label", "quotation":"text"}, inplace=True)
test.rename(columns={"code":"label", "quotation":"text"}, inplace=True)

df['label'] = [0 if x=='non-technical' else 1 for x in df['label']]
test['label'] = [0 if x=='non-technical' else 1 for x in test['label']]
...
args = TrainingArguments(
    ...
    save_strategy = "epoch",
    learning_rate=6.214868858400708e-05,
    per_device_train_batch_size=8,
    per_device_eval_batch_size=batch_size,
    num_train_epochs=9,
    weight_decay=0.16876138510420885,
    warmup_steps=250,
...
for fold_i in range(len(folds)):
  ...
  aug_train = gen_eda(train_df, output_train, 0.05, 0.1, 0.05, 0.05, 8)
